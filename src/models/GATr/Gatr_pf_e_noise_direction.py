...
from gatr.interface import embed_point, extract_scalar, extract_point, embed_scalar, extract_translation,embed_translation
import torch
...
class ExampleWrapper(L.LightningModule):
    def __init__(
        ...
        self.clustering = nn.Linear(6, self.output_dim - 1, bias=False)
    ...
    def forward(self, g, y, step_count, eval="", return_train=False,use_gt_clusters=False):
        if not use_gt_clusters:
            ...
            + embed_translation(g.ndata["pos_pxpypz_at_calo"])
            )
            ...
            vector = extract_translation(embedded_outputs[:, 0, :])
            ...
            x_v = torch.cat(
                (points.view(-1, 3), vector.view(-1, 3)), dim=1
            )
            x_cluster_coord = self.clustering(x_v)
