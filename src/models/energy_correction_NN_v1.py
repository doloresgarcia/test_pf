...
class EnergyCorrectionWrapper(torch.nn.Module):
    ...
    def neutral_prediction(self, graphs_new, neutral_idx, features_neutral_no_nan):
        ...
        if len(neutral_idx) > 0:
            ...
            if self.pid_channels:
                neutral_energies += [ torch.tensor([]).to(graphs_new.ndata["h"].device) ]
            neutral_pxyz_avg = neutral_energies
...
class EnergyCorrection():
    ...
    def forward_correction(self, g, x, y, return_train):
        ...
        pred_energy_corr[charged_idx.flatten()] = (
            charged_energies.float() #/ sum_e.flatten()[charged_idx.flatten()]
        )
        pred_energy_corr[neutral_idx.flatten()] = (
            neutral_energies.float() #/ sum_e.flatten()[neutral_idx.flatten()]
        )
        if self.fake_score_network:
            score_object = pred_pid.clone()
            if len(charged_idx):
                score_object[charged_idx.flatten()] = charged_score_pred.float()
            if len(neutral_idx):
                score_object[neutral_idx.flatten()] = neutral_score_pred.float()
        

        if len(self.pids_charged):
            if len(charged_idx):
                charged_PID_pred1 = np.array(self.pids_charged)[np.argmax(charged_PID_pred.float().cpu().detach(), axis=1)]  #0,1,2
            else:
                charged_PID_pred1 = []
        ...
        if len(self.pids_neutral):
            if len(neutral_idx):
                neutral_PID_pred1 = np.array(self.pids_neutral)[np.argmax(neutral_PID_pred.float().cpu().detach(), axis=1)] #0,1
            else:
                neutral_PID_pred1 = []
        ...
        if self.args.regress_pos:
            ...
            if len(charged_idx):
                pred_ref_pt[charged_idx.flatten()] = charged_ref_pt_pred.float().to(pred_ref_pt.device)
                pred_pos[charged_idx.flatten()] = charged_positions.float().to(pred_pos.device)
            if len(neutral_idx):
                pred_ref_pt[neutral_idx.flatten()] = neutral_ref_pt_pred.float().to(neutral_idx.device)
    ...
    def get_loss(self, batch_g, y, result, stats, fixed):
        ...
        if not dist.is_available() or not dist.is_initialized() or dist.get_rank() == 0:
            wandb.log({
                ...
                "loss regression": loss_EC_neutrals+loss_neutral_pid + loss_charged_pid 
            })
        ########### loss score ###########
        if self.fake_score_network:
